  ...
  <div>
      ...
      <tr>
  			...
  			<c:forEach var="preDay" begin="${preLastDay-(startWeek-2)}" end="${preLastDay}">
  				<td style="color:gray; font-size:0.6em">${prevYear}-${prevMonth+1}-${preDay}</td>
	  			<c:set var="cnt" value="${cnt+1}"/>
  			</c:forEach>
  			
  			<!-- 해당 월에 대한 날짜를 반복출력 -->
  			<c:forEach begin="1" end="${lastDay}" varStatus="st">
  				<c:set var="todaySw" value="${toYear==yy && toMonth==mm && toDay==st.count ? 1 : 0}"/><!-- 오늘 날짜가 맞으면 1. 틀리면 0 반환 -->
  				...
  				<c:if test="${cnt % 7 == 0}"></tr><tr></c:if> <!-- 한주가 꽉 차면 다음줄로 줄 바꾸기  </tr><tr> : 줄바꿈-->
  				<c:set var="cnt" value="${cnt+1}"/>
   			</c:forEach>
  			
  			<!-- 마지막일 이후를 다음달의 일자로 처리 -->
