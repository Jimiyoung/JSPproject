...
<head>
  ...
  <script>
  	function noticeDelCheck() {
			...
			if(ans){
				location.href="noticeDeleteOk.no?idx=${vo.idx}";
			}
		}
  </script>
  <style>
...
<p><br/></p>
<div class="container">
  <h2 class="text-center">공지사항 - 글 내 용 보 기 </h2>
  <br/>
  <table class="table table-bordered">
	  ...
	  <tr>
	  	<td colspan="6" class="text-center">
		  	<c:if test="${flag=='s'}">
		  		<input type="button" value="돌아가기" onclick="location.href='noticeSearch.no?pag=${pag}&pageSize=${pageSize}&search=${search}&searchString=${searchString}';" class="btn btn-primary"/>
		  	</c:if>	
		  	<c:if test="${flag!='s'}">
		  		<input type="button" value="돌아가기" onclick="location.href='noticeList.no?pag=${pag}&pageSize=${pageSize}';" class="btn btn-primary"/>
		  	</c:if>	
	  		<c:if test="${sLevel==0}">
		  		<input type="button" value="수정" onclick="location.href='noticeUpdate.no?idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}';" class="btn btn-success"/>
