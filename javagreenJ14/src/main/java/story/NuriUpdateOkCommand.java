...
public class NuriUpdateOkCommand implements NuriInterface {
	...
	// 전송폼의 값 모두 받아오기
  		int fSize = multipartRequest.getParameter("fileSize") ==null? 0 : Integer.parseInt(multipartRequest.getParameter("fileSize"));  // request 대신 multipartRequest
			...
			int res = dao.setNuriUpdateOk(vo);
			
			if(res==1) {
				request.setAttribute("msg", "NuriUpdateOk");
			}
			else {
				request.setAttribute("msg", "NuriUpdateNo");
			}
			request.setAttribute("url", request.getContentType()+"/nuriContent.nu?idx="+idx+"&pag="+pag+"&pageSize="+pageSize);
