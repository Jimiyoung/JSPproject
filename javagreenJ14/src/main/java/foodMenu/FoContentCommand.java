...
public class FoContentCommand implements FoodMenuInterface {
...
//선택한 게시글(idx)의 전체 내용 가져오기
	...
	//이전글과 다음글 처리(이전글/다음글의 타이틀을 가져온다.) 
			FoodMenuVO preVo = dao.getPreNextSearch("pre",idx);
			FoodMenuVO nextVo = dao.getPreNextSearch("next",idx);
